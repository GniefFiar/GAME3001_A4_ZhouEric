...
{
	TextureManager::Instance().load("../Assets/sprites/RangedEnemy.png", "RE");

	auto size = TextureManager::Instance().getTextureSize("RE");
	setWidth(size.x);
	setHeight(size.y);
...
void RangedEnemy::draw()
{
	...
	{
		// draw the ship
		TextureManager::Instance().draw("RE", x, y, getCurrentHeading(), 255, true);
